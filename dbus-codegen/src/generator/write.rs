...
pub(super) struct Stream<'a>(pub &'a mut dyn std::io::Write);
impl std::ops::AddAssign<&str> for Stream<'_> {
    fn add_assign(&mut self, rhs: &str) {
        self.0.write_all(rhs.as_bytes()).expect("Failed to write to stream");
    }
}
...
pub (super) fn module_header(s: &mut Stream, opts: &GenOpts) {
...
fn write_method_signature(s: &mut Stream, m: &Method, opts: &GenOpts, context: GenContext) -> Result<(), Box<dyn error::Error>> {
...
pub (super) fn intf_name(s: &mut Stream, i: &Intf) -> Result<(), Box<dyn error::Error>> {
...
fn write_server_access(s: &mut Stream, i: &Intf, saccess: ServerAccess, minfo_is_ref: bool) {
...
pub (super) fn intf_client(s: &mut Stream, i: &Intf, opts: &GenOpts) -> Result<(), Box<dyn error::Error>> {
