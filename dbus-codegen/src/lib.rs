...
pub use crate::generator::{
    ArgumentDefinition, ConnectionType, CreatedItems, GenOpts, Generator, InterfaceDefinition, MethodDefinition, PropertyDefinition,
    ServerAccess, SignalDefinition,
};
...
pub fn generate(xmldata: &str, opts: &GenOpts) -> Result<String, Box<dyn std::error::Error>> {
    Generator::from_string(xmldata)?.output_to_string(opts).map(|(s, _)| s)
}
